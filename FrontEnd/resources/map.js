...
app.controller('customerHomeController',function($scope, $http,$window) {
			...
			$http({
		        ...
		        url: '/customerRideHistory'
		    	  ...
		    	  angular.forEach(response.data,function(rides)
		    		{
		    		  $scope.rides.push({pickUpDate:new Date(rides.pick_up_date).toISOString(),driver:rides.driver_last_name,fare:rides.total_fare,city:rides.destination_city}); 
		    		});
		      },function(response){
		      alert(response.data);
		      });
